# Satellite Live Viewer

現在運用中の衛星の位置・高度・速度を、Webブラウザ上の地図にリアルタイムで可視化する簡易的なアプリケーションです。

※ 本プログラムはQZSS Viewerの後継プロジェクトです。
---

## 機能・特徴

*   **リアルタイムトラッキング**: 衛星の現在位置を地図上にプロットし、1秒ごとにアニメーションを更新します。
*   **軌跡表示**: 過去18時間分の衛星の軌跡を地図上にプロットし、衛星の軌道を視覚化できます。
...
It is a simple application that visualises the current position(Latitude,Longitude), altitude, and speed of satellites in real time on a map online web browser.

---

## Specifications and Features

* **Real-time tracking**: The current position of each satellite is plotted on a map with animation updated every second.
* **Orbit display**: Plot the satellite's orbit for the past 18 hours on a map to visualise the satellite orbit.
